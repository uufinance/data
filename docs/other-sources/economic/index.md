# Economic & Macroeconomic Data

Time series databases, country-level indicators, and text-based uncertainty indices for economics and finance research.

| Topic | Description |
|-------|-------------|
| [Time Series & APIs](../economic-data.md) | FRED, ECB, Bundesbank, IMF, OECD, BIS — major APIs for economic time series |
| [Country-Level Data](../macro-country-data.md) | World Bank, Penn World Table, macro history, country distances, governance |
| [Policy Uncertainty](../economic-policy-uncertainty.md) | EPU, GPR, WUI, and 50+ text-based uncertainty indices (Baker, Bloom & Davis) |

---
**See also**: [Sentiment & Culture](../sentiment-culture.md) for investor sentiment surveys and cultural dimensions | [ESG, Climate & Sustainable Finance](../esg/index.md) for climate scenarios and emissions data | [Survey Data](../survey-data.md) for household and consumer surveys.
